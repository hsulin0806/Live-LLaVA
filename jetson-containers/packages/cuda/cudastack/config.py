    ...
    if IS_TEGRA:
        if L4T_VERSION.major >= 39:  # JetPack 6 on Orin
            ...
            if CUDA_VERSION >= Version('13.2'):
                ...
                # On JP7 Tegra + CUDA 13.0, local cuDNN installers can be unavailable.
                # Use CUDA network repo packages instead of local cudnn .deb.
                cudnn_ver = '9.14.0'
                cudnn_url = ''
                cudnn_packages = "libcudnn9-cuda-13 libcudnn9-dev-cuda-13 libcudnn9-samples"
                tensorrt_ver = '10.13'
...
def cuda_stack(name, with_tensorrt=False, minimal=False, requires=None):
    ...
    # cuSPARSELt isn't currently published for JP7 Tegra CUDA 13.0 repos.
    # Keep CUDA 13 path working by skipping it on <13.2 where package lookup fails.
    if IS_TEGRA and CUDA_VERSION < Version('13.2'):
        build_args['WITH_CUSPARSELT'] = '0'
