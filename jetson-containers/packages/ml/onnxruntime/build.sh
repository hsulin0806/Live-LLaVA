...
detect_cuda_max_jobs() {
    ...
    # Clamp between reasonable bounds (Thor/CUDA13 often OOM with high parallelism)
    if [ $safe_jobs -lt 2 ]; then safe_jobs=2; fi
    if [ $safe_jobs -gt 4 ]; then safe_jobs=4; fi  # Cap at 4 for ORT CUDA build stability
