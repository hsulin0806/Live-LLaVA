...
def resolve_dependencies(packages, check=True, skip_packages=[]):
    ...
    def add_depends(packages):
        def is_skipped(name: str) -> bool:
            return any(fnmatch.fnmatch(name, skip) or fnmatch.fnmatch(name.split(':')[0], skip) for skip in skip_packages)

        def dep_version(name: str):
            if ':' not in name:
                return None
            try:
        ...
        # prune skipped packages before this iteration to avoid re-expansion churn
        packages = [p for p in packages if not is_skipped(p)]
        packages_org = packages.copy()

        for package in packages_org:
            if is_skipped(package):
                continue
            for dependency in find_package(package).get('depends', []):
                if is_skipped(dependency):
                    continue
                package_index = packages.index(package)
                dep_base = dependency.split(':')[0]
                dep_has_tag = (':' in dependency)

                # track all existing entries for this dependency base (e.g. llvm / llvm:22)
                same_base_idx = [i for i, existing in enumerate(packages) if existing.split(':')[0] == dep_base]

                if not same_base_idx:
                    # dependency not in list, add it before the package
                    packages.insert(package_index, dependency)
                    return packages, True

                # choose which existing entry should represent this dependency base
                chosen_idx = same_base_idx[0]
                chosen_val = packages[chosen_idx]

                # if current dependency is specific (tagged), prefer it over generic/base alias
                if dep_has_tag and chosen_val != dependency:
                    # if both are tagged variants (ex: llvm:20 vs llvm:22), pick a stable winner
                    if ':' in chosen_val:
                        dep_v = dep_version(dependency)
                        cur_v = dep_version(chosen_val)
                        if dep_v is not None and cur_v is not None:
                            winner = dependency if dep_v >= cur_v else chosen_val
                            if winner != chosen_val:
                                packages[chosen_idx] = winner
                                return packages, True
                            continue

                    # replace generic alias with specific tag (or normalize to requested tag)
                    packages[chosen_idx] = dependency
                    chosen_val = dependency

                    # remove duplicate entries of same base after normalization
                    dedup_seen = False
                    deduped = []
                    for x in packages:
                        if x.split(':')[0] != dep_base:
                            deduped.append(x)
                            continue
                        if not dedup_seen:
                            deduped.append(x)
                ...
                # if dependency is generic and a tagged/specific version already exists,
                # keep the specific one and do not churn aliases
                if not dep_has_tag and ':' in chosen_val:
                    pass

                # ensure the dependency appears before current package in order
                chosen_idx = packages.index(chosen_val)
                if chosen_idx > package_index:
                    packages.pop(chosen_idx)
                    packages.insert(package_index, chosen_val)
                    return packages, True

                # remove duplicates of same base to prevent oscillation/recursion
                base_count = sum(1 for x in packages if x.split(':')[0] == dep_base)
                if base_count > 1:
                    dedup_seen = False
                    deduped = []
                    for x in packages:
